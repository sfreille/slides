  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "ojivas",
        fluidRow(
          box(
            title = "Ojiva Acumulativa de Egresados",
            width = 12,
            status = "primary",
            solidHeader = TRUE,
            fluidRow(
            ...
            plotlyOutput("plot_ojiva", height = "650px")
          )
        )
      ),

      # ═══════════════════════════════════════════════════════════════════════
      # TAB 3: COMPARACIÓN (4x1 LAYOUT CON CURVAS NORMALIZADAS)
      # ═══════════════════════════════════════════════════════════════════════
      tabItem(tabName = "comparacion",
        # Gráfico 1: Años Promedio (CON CURVA NORMALIZADA)
        fluidRow(
          box(title = "Años Promedio de Egreso (Observado vs Normalizado)",
              width = 12, status = "primary",
              solidHeader = TRUE,
              plotlyOutput("plot_anos_promedio", height = "500px"),
              p("Línea sólida: Años promedio observados | Línea punteada: Proyección normalizada"))
        ),
        # Gráfico 2: Tasa de Egreso (CON CURVA NORMALIZADA)
        fluidRow(
          box(title = "Tasa de Egreso Total (Observada vs Normalizada)",
              width = 12, status = "success",
              solidHeader = TRUE,
              plotlyOutput("plot_tasa_egreso", height = "500px"),
              p("Barras sólidas: Tasa observada | Línea punteada: Proyección normalizada"))
        ),
        # Gráfico 3: Retención Y1→Y2
        fluidRow(
          box(title = "Tasa de Retención Y1 → Y2", width = 12, status = "info",
              solidHeader = TRUE, plotlyOutput("plot_retension_y1y2", height = "500px"))
        ),
        # Gráfico 4: Retención Y2→Y3
        fluidRow(
          box(title = "Tasa de Retención Y2 → Y3", width = 12, status = "warning",
              solidHeader = TRUE, plotlyOutput("plot_retension_y2y3", height = "500px"))
        )
      ),
...
server <- function(input, output, session) {
  ...
  output$plot_ojiva <- renderPlotly({
    ...
    p %>% layout(
      title = paste("Ojiva Acumulativa", titulo_extra),
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # 2. GRÁFICO: AÑOS PROMEDIO (CON CURVA NORMALIZADA PUNTEADA)
  # ─────────────────────────────────────────────────────────────────────────────

  output$plot_anos_promedio <- renderPlotly({
    datos_obs <- anos_filtrados()

    if (nrow(datos_obs) == 0) {
      return(plot_ly() %>% layout(title = "Sin datos"))
    }

    # Obtener propuestas únicas en datos filtrados
    propuestas_unicas <- unique(datos_obs$propuesta)
    ...
    # Agregar línea observada (sólida)
    p <- p %>% add_trace(
      data = datos_obs,
      x = ~anio_ingreso,
      y = ~anos_prom_observado,
      color = ~propuesta,
      type = 'scatter',
      mode = 'lines+markers',
      name = "Observado",
    ...
    # Agregar línea normalizada (punteada)
    p <- p %>% add_trace(
      data = datos_obs,
      x = ~anio_ingreso,
      y = ~anos_prom_normalizado,
      color = ~propuesta,
      type = 'scatter',
      mode = 'lines',
      name = "Normalizado",
    ...
    p %>% layout(
      title = "Años Promedio de Egreso (Observado vs Normalizado)",
      xaxis = list(title = "Año de Ingreso"),
      yaxis = list(title = "Años Promedio"),
      hovermode = "x unified",
      legend = list(x = 0.5, y = -0.3, orientation = "h", xanchor = "center", yanchor = "top"),
      margin = list(b = 120)
    )
  })

  # ─────────────────────────────────────────────────────────────────────────────
  # 3. GRÁFICO: TASA DE EGRESO (CON CURVA NORMALIZADA PUNTEADA)
  # ─────────────────────────────────────────────────────────────────────────────

  output$plot_tasa_egreso <- renderPlotly({
    datos_obs <- cohortes_filtrados() %>% filter(!is.na(prop_egr_y1))
    ...
    # Línea normalizada (punteada)
    if (!all(is.na(datos_obs$prop_egr_y1_normalizada))) {
      p <- p %>% add_trace(
        data = datos_obs %>% filter(!is.na(prop_egr_y1_normalizada)),
        x = ~anio_ingreso,
        y = ~prop_egr_y1_normalizada,
        color = ~propuesta,
        type = 'scatter',
        mode = 'lines+markers',
        name = "Normalizado",
    ...
    p %>% layout(
      title = "Tasa de Egreso Total (Observada vs Normalizada)",
      xaxis = list(title = "Año de Ingreso"),
      yaxis = list(title = "Proporción", range = c(0, max(1, max(datos_obs$prop_egr_y1, na.rm = TRUE) * 1.1))),
      hovermode = "x unified",
      legend = list(x = 0.5, y = -0.3, orientation = "h", xanchor = "center", yanchor = "top"),
      margin = list(b = 120)
    )
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # 4. GRÁFICO: RETENCIÓN Y1→Y2 (CON FILTROS)
  # ─────────────────────────────────────────────────────────────────────────────

  output$plot_retension_y1y2 <- renderPlotly({
    datos <- cohortes_filtrados() %>% filter(!is.na(tasa_ret_y1y2))

    if (nrow(datos) == 0) {
      return(plot_ly() %>% layout(title = "Sin datos"))
    }

    plot_ly(datos, x = ~anio_ingreso, y = ~tasa_ret_y1y2, color = ~propuesta,
            type = 'scatter', mode = 'lines+markers', marker = list(size = 8)) %>%
      layout(
        title = "Tasa de Retención Y1 → Y2",
        xaxis = list(title = "Año de Ingreso"),
        yaxis = list(title = "Tasa", range = c(0, 1)),
        hovermode = "x unified",
        legend = list(x = 0.5, y = -0.3, orientation = "h", xanchor = "center", yanchor = "top"),
        margin = list(b = 120)
      )
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # 5. GRÁFICO: RETENCIÓN Y2→Y3 (CON FILTROS)
  # ─────────────────────────────────────────────────────────────────────────────

  output$plot_retension_y2y3 <- renderPlotly({
    datos <- cohortes_filtrados() %>% filter(!is.na(tasa_ret_y2y3))

    if (nrow(datos) == 0) {
      return(plot_ly() %>% layout(title = "Sin datos"))
    }

    plot_ly(datos, x = ~anio_ingreso, y = ~tasa_ret_y2y3, color = ~propuesta,
            type = 'scatter', mode = 'lines+markers', marker = list(size = 8)) %>%
      layout(
        title = "Tasa de Retención Y2 → Y3",
        xaxis = list(title = "Año de Ingreso"),
        yaxis = list(title = "Tasa", range = c(0, 1)),
        hovermode = "x unified",
        legend = list(x = 0.5, y = -0.3, orientation = "h", xanchor = "center", yanchor = "top"),
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # 6. GRÁFICO: PREDICCIÓN (MODELO LOGÍSTICO)
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # 8. TABLA: DATOS COMPLETOS (CON FILTROS)
  # ─────────────────────────────────────────────────────────────────────────────
